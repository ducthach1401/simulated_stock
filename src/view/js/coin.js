...
async function getUser() {
  ...
  temp.innerHTML =
    "Money: " +
    data.money.toLocaleString("en-US", { style: "currency", currency: "USD" });
  document.getElementById("information").appendChild(temp);
  ...
  temp.innerHTML =
    "Earning: " +
    data.earning.toLocaleString("en-US", {
      style: "currency",
      currency: "USD",
    });
  document.getElementById("information").appendChild(temp);
  ...
  for (let stock of data.stockCode) {
    ...
    temp.innerHTML = (
      Math.round(cost[stock.code][1] * stock.weight * 0.999 * 10000) / 10000
    ...
    temp.innerHTML = (
      Math.round(
        (cost[stock.code][1] * stock.weight * 0.999 - stock.capital) * 10000
      ) / 10000
    ).toLocaleString("en-US");
    row.appendChild(temp);
    ...
    percent.innerHTML =
      Math.round((tempCost / stock.capital) * 10000) / 100 + " %";
...
async function updateTableInfo(cost) {
  ...
  for (let stock of user.stockCode) {
    ...
    temp.innerHTML = (
      Math.round(cost[stock.code][1] * stock.weight * 0.999 * 100) / 100
    ).toLocaleString("en-US");
    row.appendChild(temp);
    ...
    temp.innerHTML = (
      Math.round(
        (cost[stock.code][1] * stock.weight * 0.999 - stock.capital) * 100
      ) / 100
    ).toLocaleString("en-US");
    row.appendChild(temp);
    percent = document.createElement("td");
...
async function totalBill() {
  ...
  for (let row = 1; row < table.childElementCount; row++) {
    ...
    if (!isNaN(weightSell) && weightSell > 0) {
      ...
      table.childNodes[row].childNodes[3].innerHTML = (
        Math.round(price * weightSell * 0.999 * 100) / 100
      ).toLocaleString("en-US");
    } else {
...
async function showStock() {
  ...
  for (let stock in data) {
    ...
    temp.innerHTML = (
      Math.round(data[stock][1] * 1000000) / 1000000
    ).toLocaleString("en-US");
    row.appendChild(temp);
    ...
    temp.innerHTML =
      (Math.round(data[stock][2] * 1000) / 1000).toLocaleString("en-US") + " %";
    row.appendChild(temp);
    ...
    temp.innerHTML =
      (Math.round(data[stock][3] * 1000) / 1000).toLocaleString("en-US") + " %";
    row.appendChild(temp);
...
async function totalCode(id) {
  ...
  if (!isNaN(weight) && weight) {
    ...
    total.innerHTML = (
      Math.round(parseFloat(weight) * currentPrice * 10000) / 10000
    ).toLocaleString("en-US");
  } else {
    total.innerHTML = "";
  }
