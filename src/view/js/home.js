...
async function getUser() {
  ...
  temp.innerHTML =
    "Money: " +
    data.money.toLocaleString("vi-VN", { style: "currency", currency: "VND" });
  document.getElementById("information").appendChild(temp);

  temp = document.createElement("p");
  temp.innerHTML =
    "Capital: " +
    data.capital.toLocaleString("vi-VN", {
      style: "currency",
      currency: "VND",
    });
  document.getElementById("information").appendChild(temp);
  ...
  temp.innerHTML =
    "Earning: " +
    data.earning.toLocaleString("vi-VN", {
      style: "currency",
      currency: "VND",
    });
  document.getElementById("information").appendChild(temp);
...
async function showTableInfo(data) {
  ...
  for (let stock of data.stockCode) {
    ...
    temp.innerHTML = (
      cost[stock.code][3] * stock.weight * 0.999 -
      stock.capital
    ).toLocaleString("vi-VN");
    row.appendChild(temp);
...
async function totalBill() {
  ...
  for (let row = 1; row < table.childElementCount; row++) {
    ...
    if (!isNaN(weightSell) && weightSell > 0) {
      ...
      table.childNodes[row].childNodes[4].innerHTML = (
        price *
        weightSell *
        0.999
      ).toLocaleString("vi-VN");
...
async function totalCode(id) {
  ...
  if (!isNaN(weight) && weight) {
    ...
    total.innerHTML = Math.round(
      parseFloat(weight) * currentPrice
    ).toLocaleString("vi-VN");
  } else {
    total.innerHTML = "";
  }
...
async function showDiv() {
  ...
  for (let code of data) {
    ...
    if (code.ratio.length == 1) {
      td.innerText =
        code.ratio.toLocaleString("vi-VN", {
          style: "currency",
          currency: "VND",
        }) + " / Stock";
    } else {
...
async function totalPrice() {
  ...
  if (weight != "" && price != "") {
    document.getElementById("CTotal").value = (
      parseInt(price) * parseInt(weight)
    ).toLocaleString("vi-vn");
  } else {
