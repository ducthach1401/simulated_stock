...
async function getUser() {
  ...
  for (let stock of data.stockCode) {
    ...
    temp.innerHTML = (
      Math.round(cost[stock.code][3] * stock.weight * 0.999 * 100) / 100
    ...
    temp.innerHTML = (
      Math.round(
        (cost[stock.code][3] * stock.weight * 0.999 - stock.capital) * 100
      ) / 100
    ).toLocaleString("en-US");
    row.appendChild(temp);
...
async function totalCode(id) {
  ...
  if (!isNaN(weight) && weight) {
    ...
    total.innerHTML = (
      Math.round(parseFloat(weight) * currentPrice * 100) / 100
    ).toLocaleString("en-US");
  } else {
    total.innerHTML = "";
  }
